...
local function Buffer_ensure(numbytes)
    if buf_pos + numbytes > buf_size then error("malformed serialized data") end
end
...
local function write_nil(_, _) Buffer_write_byte(247) end

local function write_boolean(value, _) Buffer_write_byte(value and 249 or 248) end
...
local function write_table(value, seen)
    ...
    for k in pairs(value) do
        if (type(k) ~= 'number' or floor(k) ~= k or k > len or k < 1) and k ~= classkey then klen = klen + 1 end
    end
...
local function deserialize_value(seen)
    ...
    elseif t == 242 then
        ...
        if classkey then instance[classkey] = class end
